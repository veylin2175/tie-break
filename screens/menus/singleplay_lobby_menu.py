...
class SingleplayLobbyMenuScreen(MenuScreen):
    def __init__(self, screen, clock, options):
        ...
        self.texts += [TextBox(text="ОДИНОЧНЫЙ РЕЖИМ", **self.title_params)]
        T_X, T_Y, T_GAP = 130, 250, 90
        text_params = self.rect_params | {"x": T_X, "font_size": 40}
        self.texts += [
            TextBox(text="Имя игрока:", y=T_Y, **text_params),
            TextBox(text="Компьютеры:", y=T_Y + T_GAP, **text_params),
        ]
        ...
        B_X, B_Y = 300, 500
        self.start_button = Button(x=B_X, y=B_Y, text="СТАРТ", **button_params)
        self.back_button = Button(x=B_X + 350, y=B_Y, text="НАЗАД", **button_params)
    ...
    def add_com_player(self):
        if len(self.players) < 6:
            self.players.append(
                {
                    "name": f"Компьютер {len(self.players)}",
                    "is_com": True,
                }
            )
        self.create_com_player_buttons()
