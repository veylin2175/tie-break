...
class MultiplayLobbyMenuScreen(MenuScreen):
    def __init__(self, screen, clock, options, isHost):
        ...
        self.password = "Текущий пароль"
        self.max_players = 6
        self.opend_slots = 0
        self.texts += [TextBox(text="ЛОББИ", **self.title_params)]
        T_X, T_Y, T_GAP = 200, 200, 60
        text_params = self.rect_params | {"x": T_X, "font_size": 30}
        self.current_player_text = TextBox(
            y=T_Y + T_GAP,
            text=f"Текущие игроки: {len(self.players)}/{len(self.players) + self.opend_slots}",
            **text_params,
        )

        self.texts += [
            TextBox(y=T_Y, text=f"IP хоста: {self.host['ip']}", **text_params),
            self.current_player_text,
            TextBox(
                y=T_Y + T_GAP * 2, text=f"Название хоста: {self.host['name']}", **text_params
            ),
            TextBox(y=T_Y + T_GAP * 3, text="ПАРОЛЬ: ", **text_params),
            TextBox(y=T_Y + T_GAP * 4, text="ВЫГНАТЬ: ", **text_params),
        ]
        # self.is_host = False
        # Host only
        if self.is_host:
            ...
            self.host_only_kick_text = TextBox(
                x=340, y=T_Y + T_GAP * 4, text="Только хост может выгонять", **text_params
            )
        ...
        ready_start_section = []
        self.ready_button = Button(x=B_X, text="ГОТОВ", **ready_start_params)
        ready_start_section.append(self.ready_button)
        if self.is_host:
            self.start_button = Button(x=B_X + 270, text="СТАРТ", **ready_start_params)
            ready_start_section.append(self.start_button)
        self.button_sections += [ready_start_section]
        self.back_button = Button(
            x=T_X - 10, y=90, text="НАЗАД", **self.rect_params, font_size=30, width=150
        )
    ...
    def draw_player(self, surface, player, is_me):
        ...
        if player["is_ready"]:
            ready_text = TextBox(text="ГОТОВ", **ready_params, text_color="green")
        else:
            ready_text = TextBox(text="НЕ ГОТОВ", **ready_params, text_color="red")
    ...
    def update_slot_text(self):
        self.current_player_text.set_text(
            f"Текущие игроки: {len(self.players)}/{len(self.players) + self.opend_slots}"
        )
