...
class EndMenuScreen(MenuScreen):
    def __init__(self, screen, clock, options, winner, game_info):
        ...
        self.winner = winner
        self.texts += [TextBox(text="ИГРА ОКОНЧЕНА", **self.title_params)]
        self.game_info = game_info
        self.winner_text = TextBox(
            text=f"{self.winner.get_name()} победил!",
            **self.rect_params,
            x="center",
            y=200,
        )
        ...
        self.home_button = Button(y=B_Y, text="ДОМОЙ", **buttons_params)
        self.quit_button = Button(y=B_Y + B_GAP, text="ВЫХОД", **buttons_params)
