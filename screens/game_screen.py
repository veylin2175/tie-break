...
class GameScreen(Screen):
    ...
    def create_hand_cards(self, screen_pos, player, is_me):
        ...
        if is_me:
            C_WIDTH, C_HEIGHT, C_X, C_Y = 80*1.5, 110*1.5, x + 80, y + 70
            ...
            C_WIDTH, C_HEIGHT, C_X, C_Y = 40*1.5, 55*1.5, x + 30, y + 50
            if len(player_hand) > 7:
    ...
    def create_card_renders(self):
        ...
        if self.game.top_special_card() is not None:
            top_special_card_render = Card(
                x=self.pos["discard"][0] + 300,
                y=self.pos["discard"][1],
                width=80 * 1.5,
                height=80 * 1.5,
                face_up=True,
                card=self.game.top_special_card(),
                **self.rect_params,
            )
            self.card_renders.append(top_special_card_render)
    ...
    def draw_direction(self, surface):
        ...
        if self.game.get_direction() == 1:
            text_params = text_params | {"text_color": "white", "text": "↓"}
        else:
            text_params = text_params | {"text_color": "white", "text": "↑"}
    ...
    def update_animations_finished(self):
        ...
        for i, animation in enumerate(self.animations):
            ...
            if animation.is_finished():
                if animation.__class__.__name__ == "MoveAnimation":
                    if animation.move_info["type"] == "card_move":
                        self.game.update_by_animation_info(animation.move_info)
            else:
    ...
    def process_game_events(self):
        for event_info in self.game.get_game_event_infos():
            if event_info["type"] == "player_win":
                ...
                self.show_message(
                    f"{event_info['value'].get_name()} крикнул матчбол!",
                    time=self.message_time,
                )
            elif event_info["type"] == "uno_failed":
                self.show_message(
                    f"{event_info['value'].get_name()} не удалось крикнуть матчбол!",
                    time=self.message_time,
                )
